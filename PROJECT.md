...
- `.header-menu-trigger` supplies the shared location/account height, radius, 12px gap (`--space-3`, same as list rows next to an avatar), transparent rest state, and subtle hover. Both use one ChevronDown which rotates while open. Its 8px inset **stacks on** the identity row's `gap-4`, so the location trigger carries `margin-inline-start: calc(-1 * var(--space-2))` — without it the marker sits 24px from the hairline while the logo sits 16px from it. Do not even the gap by shrinking `gap-4` or padding the logo; the pull-back keeps the hover fill full-size. This mirrors the account trigger's `-mr-4` at the other end of the row.
